...
::: {.incremental}
- Recap: differentiable simulators, losses, and constrained parameter learning
- Revisiting continuous-time state-space models through differentiable solvers
- Fixed-step ODE solvers as differentiable program blocks
- Neural ODEs as learnable continuous-time dynamics on the audio grid
- Backpropagating through trajectories: direct AD, adjoints, and practical caveats
- Hybrid physical-neural models for modal sound synthesis
- Mini task: learn a missing force or residual from modal trajectories
- Stability, energy, stiffness, and solver choices for audio-rate systems
- Bridge to next week: physics-informed losses and residual constraints
:::
...
## Mini Task: Learn a Missing Residual

Target:

::: {.incremental}
- simulate a modal oscillator with a known nonlinear force
- hide the nonlinear force from the model
- keep the linear modal backbone
- train a small residual network to match the target trajectory
:::
...
Listen, plot trajectories, and inspect the learned residual.
