...
This can reconstruct a signal well, but it may not be easy to recover the actual physical parameters.

First we need to know the exact number of modes; even then, different modal descriptions can fit the same observation.
...
where $\tau = \frac{T_0}{\rho A_{\mathrm{cs}}}$ and $B = \frac{EI}{\rho A_{\mathrm{cs}}}$.
...
Let $y=(y_1,\ldots,y_T)$ be one observed waveform, with $t$ indexing samples.

. . .

Assume an additive Gaussian noise model:
...
Under this noise model, maximising likelihood is equivalent to minimising waveform MSE.
...
A loss-only estimate asks for parameters that best fit the data:

$$
\hat\theta
=
\arg\min_\theta
\mathcal L_{\mathrm{data}}(g(\theta),y).
$$

. . .

A prior adds a penalty for implausible parameters:
...
This is still a point estimate: one best trade-off between data fit and physical plausibility.

. . .

If both terms come from probability models, this is MAP estimation.
...
This describes which regions of parameter space plausibly explain the sound.

. . .

![](images/posterior_multimodal.svg){width=56% fig-align="center"}
...
A conditional flow learns an approximation:
...
To sample plausible parameters:
...
The conditioning vector comes from the observation:
...
$$
\log p_\phi(\theta_{\mathrm{uncon}}\mid y)
=
\log p(z)
+
\log
...
- Can good samples initialise differentiable optimisation?
